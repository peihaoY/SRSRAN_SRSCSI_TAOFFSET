...
def recv_csi_udp(listen_ip="0.0.0.0", listen_port=5000):
    ...
    while True:
        ...
        time_alignment_s, timestamp = struct.unpack("<dQ", data[float_region_len:float_region_len+16])
        ...
        print(f"RX={rx_port}, TX={tx_port}, "
              f"TA={time_alignment_s * 1e6:.3f} µs, "
              f"CSI_len={len(csi)}, "
        ...
        if len(save_buffer) >= SAVE_EVERY_N:
            ...
            filename = os.path.join(output_dir, f"csi_snapshot_{timestamp_now}.npz")
